...
- `suggestedFilename`: slug defaults to `"license"` if null; the `jti` is shortened by stripping the `lic_` prefix and taking the next 8 hex chars (`substring(4, 12)`) only when it actually starts with `lic_` and is long enough. So `lic_0192abcd...` → `acme-0192abcd.lic`.
